   ...
   if(s.direction>0)
     {
      ...
      else ok=g_trade.BuyLimit(lots,s.entry,_Symbol,s.sl,s.tp2,ORDER_TIME_SPECIFIED,exp,0,InpTradeComment);
     }
   else
     {
      double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
      if(bid>=s.entry) ok=g_trade.Sell(lots,_Symbol,0,s.sl,s.tp2,InpTradeComment);
      else ok=g_trade.SellLimit(lots,s.entry,_Symbol,s.sl,s.tp2,ORDER_TIME_SPECIFIED,exp,0,InpTradeComment);
     }
